...
export async function GET(request: Request, { params }: Props) {
  try {
    ...
    if (!reserva) {
      return new NextResponse(
        HTML_TEMPLATE(
          ...
          'La reserva que intentás cancelar no existe en el sistema.',
          '#fef2f2',
          '❌'
        ),
    ...
    return new NextResponse(
      HTML_TEMPLATE(
        'Reserva Cancelada',
        `La reserva de <strong>${updated.nombre} ${updated.apellido}</strong> para el <strong>${updated.fecha.toLocaleDateString('es-AR')}</strong> a las <strong>${updated.hora}</strong> ha sido cancelada. Se envió un email al cliente.`,
        '#fee2e2',
        '🚫'
      ),
      { headers: { 'content-type': 'text/html' } }
